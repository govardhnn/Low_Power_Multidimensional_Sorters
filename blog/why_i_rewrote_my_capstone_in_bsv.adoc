== A Compelling Case for using BSV in Academia 
...
At link:https://incoresemi.com/[InCore], the use of Bluespec SystemVerilog is one of the 
...
Back then, specifying designs dealing with parallelism and concurrency was 
always a challenging and discouraging due to lower levels of abstraction.

A year ago, as a novice BSV designer, I decided it would be meaningful
...
. Sort 64 elements passed as a 8x8 array in six phases,
. Discover a 25% reduction in gate-count (121k to 90k) by comparing the legacy 
Verilog implementation vs the BSV implementation using a simple optimization.
...
Ultimately, an ideal test case where all 64 inputs specified in descending order in
```
