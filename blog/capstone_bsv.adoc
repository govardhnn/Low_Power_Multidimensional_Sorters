...
Like most undergraduate students, I started out designing my initial digital
...
always challenging due to its structural and timing emphasis on thinking "how" the hardware designs look like instead of "what" rules, conditions and invariants need to be specified to ensure correct behavior, leaving the scheduling to the Bluespec compiler.

For example, implementing a FIFO buffer:
...
from my capstone project in a High-Level Hardware Description Language - BSV.

Before addressing the timing and area concerns of this redesign, 
the focus of this work is to appreciate if the BSV counterpart
...
. Discover a 25% reduction in gate-count (121k to 90k) by comparing the legacy 
...
Here, the `mav_get_sort` output will only compare the two 32-bit inputs
to the sorter, i.e. check if the first input `mav_get_sort_cae_in[31:0]`
is less than or equal to the second input `mav_get_sort_cae_in[63:32]` and 
return the output as `mav_get_sort_cae_in`, else return the
ascending order: `{ mav_get_sort_cae_in[31:0], mav_get_sort_cae_in[63:32] }`.
...
The MDSA algorithm efficiently uses Parallel Hardware Sorter Arrays(PHSAs) like the Bitonic sorter we earlier designed to specify an architecture that uses eight such units 
to sort 64 elements in 6 stages by simply alternating between row and column sorting, and rerouting the order of outputs (ascending/descending).
...
=== Steps to generate verilog and run simulations of the CAE, BM4, BM8 and MDSA_Bitonic in the GitHub Repository
...
Many thanks to the peers at InCore who provided valuable feedback on early drafts of this blog.
...
Initial Work on MDSA[5] and PHSA sorter variants[6] have been the guiding light for the
theory that has led to the innovation in the field of hardware sorting. 
