...
. To run the simulation with the bluespec compiler
...
The block diagrams and drawings to aid the explanation of the CAE, Bitonic and MDSA are from the paper[1], and the legacy Verilog codebase from the team (mailto:sa.samahith@gmail.com[Samahith S A ], mailto:treja.manu@gmail.com[Manogna R], mailto:hiteshramaswamy@gmail.com[Hitesh D ]) guided by my UG Professor - (mailto:kumar.sudeendra@gmail.com[Dr. Sudeendra Kumar]) at PES University, Bangalore.

The BSV language [] [] compiler and libraries reference guide from Bluespec(inc)
...
. link:The Bluespec Compiler(bsc)[https://github.com/B-Lang-org/bsc] and the 
